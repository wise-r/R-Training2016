...
在课程的开始，希望大家的电脑上已经安装了[火狐浏览器](http://www.firefox.com.cn/)或者深得我心，但需要翻墙才能使用的谷歌浏览器[chrome](http://www.baidu.com/link?url=Uinb_ahhfpq1DmBRXiRxPlwNtNvLBH0NHyyy425DQ38bPDdi_wbZiTf5E6UNjZB5&wd=&eqid=bc164beb000300810000000659037b6e)!!!
...
严格按照预定义标签来构造HTML文件，且必须严格按照下面的框架构造HTML文件：
...
![](C:/Users/hufan/Documents/研一下/WiseRClub/img.jpg)
...
- 表单标签`<form>`，后面我们将利用rvest包创建会话，填写这个表单，实现表单穿越。在精简过知乎的登录表单后，下面的形式应该可以实现简单的表单设计：
...
![](C:/Users/hufan/Documents/研一下/WiseRClub/form.jpg)


- 表格标签`<table>`，这个标签里通常还嵌套了表格的表头标签`<th>`，表格的行标签`<tr>`。而`<tr>`标签里又嵌套了单元格数据标签`<td>`，`<td>`标签存放了表格的单元格数据。在HTML里的格式为：
...
**Note**：大家可以把上面的代码嵌套入严格的HTML框架中，放入记事本或者Notepad++中，保存后用浏览器打开，就能看出是不是一个表格了：


![](C:/Users/hufan/Documents/研一下/WiseRClub/table2.jpg)
...
![](C:/Users/hufan/Documents/研一下/WiseRClub/ul.jpg)

无序列表标签`<ol>`与`<ul>`的表示方法差不多，只需要将上面的`<ul>`，`</ul>`标签变成`<ol>`，`</ol>`这两个标签，其他的标签保持不变，就会得到下面的效果：

![](C:/Users/hufan/Documents/研一下/WiseRClub/ol.jpg)
...
![](C:/Users/hufan/Documents/研一下/WiseRClub/dl.jpg)

其实描述列表标签的效果就相当于RMarkdown中的这种形式：
...
JSON与XML都属于数据格式，通常被用来存储数据以及进行数据交换。而JSON是当前最流行的格式之一，JSON存储数据的格式如下：

![](C:/Users/hufan/Documents/研一下/WiseRClub/json2.jpg)

它在HTTP响应中是如下格式：

![](C:/Users/hufan/Documents/研一下/WiseRClub/json.jpg)

实际上JSON就是用键值对来存储数据，键值对，英文名称是`key:value`，就相当于`key=value`。

#### 1.2.2.2 XML格式

XML和HTML长得很像，但是与HTML不同的是，XML没有预定义的标签，即没有`<div>`、`<p>`、`<ul>`等预定义标签，但你可以自定义标签，你可以定义为`<叫姐姐 听话="有糖吃">`、`<叫阿姨 不听话="好气哦，还是要保持微笑">`。XML用标签内容来存储数据。

### 1.3 Rvest 包入门

可能有同学已经使用过`Rcurl`包，它是一个强大的爬虫包，但是包里面的函数不是很容易记忆。rvest包的函数基本看过一遍就能记住，虽然它相当于Rcurl的轻量级爬虫包，但是已经足够我们爬取静态网页等大部分爬取工作，据Hadley大神说，rvest包的开发，是受到了 Python 的著名爬虫库`requests`的启发。
...
#### 案例一：爬取包含所有 R 包的 CRAN 网页内容

- 抓取网页数据

![不包含表头标签的table标签](C:/Users/hufan/Documents/研一下/WiseRClub/table标签.jpg)
...
View(df)
```
其实我们不需要写循环来完成这件事，用`lapply()`也可以完成这件事，只不过这时`lapply()`函数的第一个`x`参数应该是一个`1:page`的向量，这样就能把每一页page传入后面的`FUN`函数中，大家可以回去试试。
...
1. 它虽然作为chrome的插件，但是用firefox的童鞋也不要灰心丧气，你只需要把SelectorGadget的指定链接（请参见 SelectorGadget 官网）拉入你的收藏夹，等到下次要查找节点的时候，点击收藏夹里的它，接下来的操作就和在chrome里装了插件一样。


2. 安装完毕后，先点击插件，然后选中你想要查找的网页内容，那么该内容会标绿。整个页面可能也会出现标黄的内容，而这些黄色部分表示的是为匹配同一节点的内容。重复点击某一标黄内容，该内容会继而被标红，这表示排除刚刚重复点击的内容。
...
str(t3)
```
**Note**：这里必须指出很重要的一点转化，列表中的列向量必须转化为行向量，才能在第5步中，使每一条成员信息都按行正确地拼接。
...
举个例子，有一条长微博，你点击了”展开全文“，但页面却没有发生跳转，网址也没有发生变化，这就是异步加载。动态网页如今基本都使用了异步加载技术：

![](C:/Users/hufan/Documents/研一下/WiseRClub/ajax.jpg)
...
`r$url`会得到我们请求资源的URL，看看和我们设定的是否一样，copy到浏览器
...
![](C:/Users/hufan/Documents/研一下/WiseRClub/auth.jpg)
